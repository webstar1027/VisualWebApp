<template>
    <div class="frais-structures">
        <el-card>
            ...
            <ApolloQuery
                    :query="require('../../../../../graphql/simulations/foyers/fraisDeStructure/foyersFraisStructures.gql')"
                    :variables="{
          simulationId: simulationID}">
                <template slot-scope="{ result:{ loading, error, data }, isLoading, query }">
                    ...
                    <el-table
                            ...
                            v-loading="isLoading"
                            style="width: 100%">
                        <el-table-column sortable fixed column-key="number" prop="number" width="170" label="N°"></el-table-column>
                        <el-table-column sortable fixed column-key="libelle" prop="libelle" width="170" label="Libellé"></el-table-column>
                        <el-table-column sortable fixed column-key="type" prop="type" width="170" label="Nature">
                            <template slot-scope="scope">
                                <p>{{renderType(scope.row.type)}}</p>
                            </template>
                        </el-table-column>
                        <el-table-column sortable fixed column-key="tauxDevolution" prop="tauxDevolution" width="150" align="center" label="Taux d'évolution"></el-table-column>
                        <el-table-column
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-6">
                            <el-form-item label="Taux d'évolution " prop="tauxDevolution">
                                <el-input type="text" v-model="form.tauxDevolution" placeholder="0" class="text-input" :disabled="!form.indexation"
...
<script>
    ...
    import {checkAllPeriodics, initPeriodic, mathInput, periodicFormatter} from '../../../../../utils/inputs';
    ...
    export default {
        ...
        data() {
            let validateLibelle = (rule, value, callback) => {
                if (value === '') {
                    callback(new Error('Vous devez saisir un libellé de frais de structure'));
            ...
            return {
                ...
                formRules: {
                    ...
                    type: [
                        { required: true, message: "Vous devez saisir la nature de frais de structure", trigger: 'change' }
                    ]
                }
        ...
        methods: {
            initForm() {
                this.form = {
                    id: null,
                    number: getIncremental(this.fraisStructures, 'number'),
                    libelle: '',
                    indexation: true,
                    type: '',
                    tauxDevolution: null,
            ...
            tableData(data, query) {
                if (!_.isNil(data)) {
                    this.query = query;
                    const fraisStructures = data.foyersFraisStructures.items.map(item => {
                        let periodiques = [];
                        let total = 0;
                        item.foyersFraisStructuresPeriodique.items.forEach(periodique => {
                    ...
                    if (fraisStructures.length > 0 ) {
                        this.currentLastNumber = parseInt(fraisStructures[fraisStructures.length - 1].number);
                    }
            ...
            handleDelete(index, row) {
                this.$confirm('Êtes-vous sûr(e) de vouloir supprimer cette ligne ?')
                    .then(_ => {
                        this.$apollo.mutate({
                            mutation: require('../../../../../graphql/simulations/foyers/fraisDeStructure/removeFoyersFraisStructure.gql'),
                            variables: {
            ...
            save(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid && checkAllPeriodics(this.form.periodiques)) {
                        this.$apollo.mutate({
                            mutation: require('../../../../../graphql/simulations/foyers/fraisDeStructure/saveFoyersFraisStructure.gql'),
                            variables: {
                                fraiStructure: {
                                    id: this.form.id,
                                    simulationId: this.simulationID,
                                    ...
                                    periodique: JSON.stringify({periodique: this.form.periodiques.fraisStructuresPeriodique})
                                }
                            }
                        }).then(() => {
            ...
            isLibelle(libelle) {
                for (let i = 0; i < this.fraisStructures.length; i++) {
                    if (this.fraisStructures[i].libelle === libelle && this.isEdit === false) {
                        return true;
                    }
                }
            },
...
<style type="text/css">
    .foyers-frais-structures .fixed-input {
        width: 80px;
    }
    .foyers-frais-structures .el-input.is-disabled .el-input__inner {
        color: black;
    }
    .foyers-frais-structures .form-slider i {
        font-size: 25px;
        margin-left: 50px;
        cursor: pointer;
    }
    .foyers-frais-structures .el-form-item__label {
        line-height: 40px;
    }
</style>
