<template>
    <div class="autres-activites">
        ...
        <el-dialog
            title="Création/modification d’une activité"
            ...
            <div slot="footer" class="dialog-footer">
                ...
                <el-button type="success" @click="submit('codificationForm')" :disabled="isSubmitting">Valider</el-button>
            </div>
        </el-dialog>
    </div>
</template>
...
<script>
    ...
    methods: {
        initForm() {
            this.form = {
                id: null,
                numero: getIncremental(this.codifications, 'numero'),
                activite: ''
            };
        },
        tableData(data, query) {
            if (!_.isNil(data)) {
                this.query = query;
                this.codifications = data.codifications.items;
                return data.codifications.items;
            } else {
                return [];
            }
        },
        ...
        handleDelete(index, row) {
            this.$confirm('Êtes-vous sûr de vouloir supprimer cette codification?')
                .then(_ => {
                    this.$apollo.mutate({
                        mutation: require('../../../../../graphql/simulations/autres-activites/removeCodification.gql'),
                        variables: {
                            ...
                            updateData(this.query, this.simulationID).then(() => {
                                this.$message({
                                    showClose: true,
                                    message: 'Cette codification a bien été supprimée.',
                                    type: 'success'
                                });
                            })
                        }).catch(error => {
        ...
        submit(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    this.$apollo.mutate({
                        mutation: require('../../../../../graphql/simulations/autres-activites/saveCodification.gql'),
                        variables: {
                            uuid: this.form.id,
                            simulationId: this.simulationID,
                        ...
                        updateData(this.query, this.simulationID).then(() => {
                            this.$message({
                                showClose: true,
                                message: 'Cette codification a bien été enregistrée.',
                                type: 'success'
                            });
                        })
                    }).catch(error => {
