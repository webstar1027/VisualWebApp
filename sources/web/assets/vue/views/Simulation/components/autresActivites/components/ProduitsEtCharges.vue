<template>
    <div class="produits-charges">
        ...
        <ApolloQuery
            ...
            <template slot-scope="{ result:{ loading, error, data }, isLoading, query }">
              ...
              <el-table
                    ...
                    style="width: 100%">
                <el-table-column sortable fixed column-key="number" prop="number" width="60" label="N°"></el-table-column>
                <el-table-column sortable fixed column-key="activite" prop="activite" width="120" label="Activite"></el-table-column>
                <el-table-column sortable fixed column-key="libelle" prop="libelle" width="120" label="Libelle"></el-table-column>
                <el-table-column sortable fixed column-key="type" prop="type" width="120" label="Nature">
      ...
      <el-dialog
        title="Création / Modification de produit et charge"
    ...
    data() {
        return {
            ...
            formRules: {
                ...
                activite: [
                    { required: true, message: "Vous devez saisir LA activite de produit et charge", trigger: 'change' }
                ]
            },
            activites: [],
            currentLastNumber: 0
    ...
    methods: {
        ...
        handleDelete(index, row) {
            this.$confirm('Êtes-vous sûr de vouloir supprimer ce produit ?')
                .then(_ => {
                    this.$apollo.mutate({
                        mutation: require('../../../../../graphql/simulations/produit-charge/removeProduitCharge.gql'),
                        variables: {
        ...
        getActivites() {
          this.activites = [];
          this.$apollo.query({
                query: require('../../../../../graphql/simulations/autres-activites/codifications.gql'),
                variables: {
                    simulationId: this.simulationID
                }
            }).then((res) => {
                let codifications = res.data.codifications.items;
        ...
        save(formName) {
            ...
            this.$refs[formName].validate((valid) => {
                if (valid && checkAllPeriodics(this.form.periodiques)) {
                    this.$apollo.mutate({
                        mutation: require('../../../../../graphql/simulations/produit-charge/saveProduitCharge.gql'),
                        variables: {
                            produitCharge: {
                                id: this.form.id,
                                simulationId: this.simulationID,
                        ...
                        updateData(this.query, this.simulationID).then(() => {
                            this.$message({
                                showClose: true,
                                message: 'Ces produit a bien été enregistré.',
                                type: 'success'
                            });
                        })
                    }).catch(error => {
